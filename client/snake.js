...
//i want to make the game faster by time so i will use settimeout like we learned in class
    setTimeout(drawGame, 1000/ speed);
} 

function isGameOver(){
    let gameOver = false;
//if game has not started that is not a game over
    if(yVelocity ===0 && xVelocity ===0){
        return false;
    }
    
    //if we hit the wals game is over
    if(headX < 0 ){
        gameOver = true;
    }
...
function clearScreen(){
    ...
    ctx.fillStyle = 'black';
    //takig the fill style and using it in our canvas
